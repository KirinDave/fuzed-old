...
start_link(Master, SerivceModule, UpRetry, DownRetry) ->
  gen_fsm:start_link({local, ?MODULE}, ?MODULE, [Master, SerivceModule, UpRetry, DownRetry], []).
...
init([Master, ServiceModule, UpRetry, DownRetry]) ->
  process_flag(trap_exit, true),
  link(global:whereis_name(ServiceModule)),
  Ref1 = pinger(UpRetry),
  {ok, up, #state{master = Master, srv_mod = ServiceModule, 
                  up_retry = UpRetry, down_retry = DownRetry, 
                  ping_ref = Ref1}}.
...
rejoin({ping, Ref}, StateData) when Ref =:= StateData#state.ping_ref ->
  case check_master_responsive(StateData#state.srv_mod) of
    ok ->
      error_logger:info_msg("Master node is responsive. Going 'up'.~n"),
      link(global:whereis_name(StateData#state.srv_mod)),
...
check_master_responsive(ServiceModule) ->
  try apply(ServiceModule, identity, []) of
    {ok, _Node} -> 
      ok;
    _ ->
      fail
